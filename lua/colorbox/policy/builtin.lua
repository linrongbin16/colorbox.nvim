...
M.shuffle = function()
  ...
  if #ColorNamesList > 0 then
    ...
    local color = policy_util.get_next_color_name_by_idx(i)
...
M.in_order = function()
  local ColorNamesList = colornames.colornames()
  if #ColorNamesList > 0 then
    local previous_track = policy_util.previous_track() --[[@as colorbox.PreviousTrack]]
...
M.reverse_order = function()
  ...
  if #ColorNamesList > 0 then
    ...
    local color = policy_util.get_prev_color_name_by_idx(i)
    vim.cmd(string.format([[color %s]], color))
  end
end
...
M.single = function()
  ...
  if #ColorNamesList > 0 then
    ...
    local color = previous_track ~= nil and previous_track.color_name
      or policy_util.get_next_color_name_by_idx(0)
