...
M.exists = function(p)
  ...
  local result, _ = require("colorbox.commons.uv").fs_lstat(p)
  return result ~= nil
end
...
M.expand = function(p)
  ...
  if string.len(p) >= 1 and string.sub(p, 1, 1) == "~" then
    return require("colorbox.commons.uv").os_homedir() .. string.sub(p, 2)
  else
    return p
  end
end
...
M.resolve = function(p)
  ...
  local result, _ = require("colorbox.commons.uv").fs_realpath(p)
  -- print(
  --   string.format(
