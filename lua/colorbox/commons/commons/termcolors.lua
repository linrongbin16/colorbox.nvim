...
M.render = function(text, name, hl)
    ...
    local fgfmt = nil
    local fgcode = strings.not_empty(hl)
            and M.retrieve("fg", hl --[[@as string]])
        or nil
    ...
    local fmt = nil
    local bgcode = strings.not_empty(hl)
            and M.retrieve("bg", hl --[[@as string]])
        or nil
...
M.erase = function(text)
    ...
    local result, pos = text:gsub("\x1b%[%d+m\x1b%[K", "")
        :gsub("\x1b%[m\x1b%[K", "")
