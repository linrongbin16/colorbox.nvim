        ...
        for _, tag in ipairs(tags) do
            ...
            if is_tag(i) then
                ...
                elseif tag == "MSECS" then
                    table.insert(
                        builder,
                        string.format(self.msecsfmt, meta.MSECS)
                    )
...
function FileHandler:write(meta)
    ...
    if self.filemode == "w" then
        assert(
            self.filehandle ~= nil,
            string.format(
                "failed to write file log:%s",
                vim.inspect(self.filepath)
            )
        )
        fp = self.filehandle
...
function Logger:_log(dbg, lvl, fmt, ...)
    ...
    for _, handler in ipairs(self.handlers) do
        ...
        local meta_info = {
            ...
            LINE_NO = dbg ~= nil and (dbg.currentline or dbg.linedefined)
                or nil,
...
M.setup = function(opts)
    ...
    if conf.file_log then
        ...
        local filepath = string.format(
            "%s%s",
            type(conf.file_log_dir) == "string"
                    and (conf.file_log_dir .. SEPARATOR)
                or "",
            conf.file_log_name
        )
        local file_handler =
            FileHandler:new(filepath, conf.file_log_mode or "a")
